...
from test_cases_ch_69 import test_inp, test_out
...
def test_challenge_69(input: Callable) -> None:
    ...
    with Capturing() as output:
        ...
        import twt_69  # change this to your file name with solution without .py extension
    ...
    for i, (out, exp) in enumerate(zip_longest(output, test_out), 1):
        j = int(test_inp[ii].split()[0])
        inp = test_inp[ii: ii + j + 1]
        ii += j + 1
        if out != exp:
            print(f"Test nr:{i}\n      Input: {cyan}")
            pprint(inp)
